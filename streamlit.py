...
if models == 'Fashion MNIST':
    ...
    if uploaded_file is not None:
        ...
        image_array = np.asarray(image_g).astype('uint8')
        image_small = cv2.resize(image_array,(28,28),interpolation=cv2.INTER_AREA)
        pre = np.argmax(model.predict(np.expand_dims(image_small,0)))
        label = {
                    0:'T-shirt/top',
                    1:'Trouser',
                    2:'Pullover',
        ...
        st.image(img, caption=f"Model predict: {cap}")
